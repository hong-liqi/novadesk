...
Definir a estratégia de observabilidade do NovaDesk: logging estruturado, métricas, distributed tracing, health checks, alertas e dashboards. Todo serviço deve ser observável desde o primeiro deploy.
...
| ------- | -------------------------------- | ----------------------------------- |
| Logs    | Pino + pacote `@novadesk/logger` | Eventos discretos, debugging        |
...
| Regra                             | Descrição                               |
| --------------------------------- | --------------------------------------- |
| Nunca `console.log`               | Usar `@novadesk/logger` exclusivamente  |
...
- [ ] `@novadesk/logger` configurado com service name
