...
Definir infraestrutura, containerização, orquestração local, pipelines CI/CD, estratégia de deploy, gestão de ambientes e procedimentos operacionais do NovaDesk.
...
| staging    | Pré-produção, demos  | `staging.novadesk.dev` | Auto em merge para `develop` |
| production | Portfólio público    | `novadesk.dev`         | Auto em merge para `main`    |
...
- Naming: `ghcr.io/{owner}/novadesk-{service}:{tag}`
